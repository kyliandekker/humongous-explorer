...
{ 
	ArgType_Byte, // Unsigned.
	ArgType_Int16, // Signed.
	ArgType_Int32, // Signed.
	ArgType_Ref, // Signed, Int16 or Int32 depending on version.
	ArgType_String // Null-terminated.
};
...
public:
	...
	//---------------------------------------------------------------------
	OPArgument(const core::Data& a_Data, ArgumentType a_eArgumentType) :
		m_Data(a_Data),
	...
	//---------------------------------------------------------------------
	void AdjustRefJump(int32_t delta)
	{
		if (m_Data.size() == sizeof(int32_t))
		{
			int32_t val = GetRefJump() + delta;
			unsigned char* bytes = m_Data.dataAs<unsigned char>();
			bytes[0] = val & 0xFF;
			bytes[1] = (val >> 8) & 0xFF;
			bytes[2] = (val >> 16) & 0xFF;
			bytes[3] = (val >> 24) & 0xFF;
		...
		{
			int16_t val = static_cast<int16_t>(GetRefJump()) + static_cast<int16_t>(delta);
			unsigned char* bytes = m_Data.dataAs<unsigned char>();
			bytes[0] = val & 0xFF;
			bytes[1] = (val >> 8) & 0xFF;
		}
...
public:
	//---------------------------------------------------------------------
	OPInstruction(uint8_t a_iByteCode) :
		m_iByteCode(a_iByteCode)
	{ }

	//---------------------------------------------------------------------
	void AddArgument(const OPArgument& a_Argument)
	{
		m_aArguments.push_back(a_Argument);
	}
	...
	{
		size_t size = 0;
		for (const OPArgument& argument : m_aArguments)
		{
	...
	const std::vector<OPArgument>& GetArguments() const
	{
		return m_aArguments;
	}

	std::vector<OPArgument>& GetArguments()
	{
		return m_aArguments;
	}
...
private:
	...
	std::vector<OPArgument> m_aArguments;
};

//---------------------------------------------------------------------
OPInstruction default_func(uint8_t byte, const unsigned char* data);
struct bytecode
{
	std::string m_sName;
	std::function<OPInstruction(uint8_t, const unsigned char*)> m_fnSize = default_func;
};

//---------------------------------------------------------------------
OPInstruction extended_b_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);

	instructions.AddArgument(
		OPArgument(core::Data(data, sizeof(uint8_t)), ArgumentType::ArgType_Byte)
	);
...
//---------------------------------------------------------------------
OPInstruction extended_w_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);

	instructions.AddArgument(
		OPArgument(core::Data(data, sizeof(uint16_t)), ArgumentType::ArgType_Int16)
	);
...
//---------------------------------------------------------------------
OPInstruction extended_ww_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(core::Data(data + sizeof(uint16_t), sizeof(uint16_t)), ArgumentType::ArgType_Int16)
	);
...
//---------------------------------------------------------------------
OPInstruction extended_dw_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);

	instructions.AddArgument(
		OPArgument(core::Data(data, sizeof(uint32_t)), ArgumentType::ArgType_Int32)
	);
...
//---------------------------------------------------------------------
OPInstruction extended_ddw_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(core::Data(data + sizeof(uint32_t), sizeof(uint32_t)), ArgumentType::ArgType_Int32)
	);
...
//---------------------------------------------------------------------
OPInstruction extended_bw_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(core::Data(data + sizeof(uint8_t), sizeof(uint16_t)), ArgumentType::ArgType_Int16)
	);
...
//---------------------------------------------------------------------
OPInstruction extended_bdw_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(core::Data(data + sizeof(uint8_t), sizeof(uint32_t)), ArgumentType::ArgType_Int32)
	);
...
//---------------------------------------------------------------------
OPInstruction jump_cmd(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);

	instructions.AddArgument(
		OPArgument(core::Data(data, sizeof(int16_t)), ArgumentType::ArgType_Ref)
	);
...
//---------------------------------------------------------------------
OPInstruction djump_cmd(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);

	instructions.AddArgument(
		OPArgument(core::Data(data, sizeof(int32_t)), ArgumentType::ArgType_Ref)
	);
...
//---------------------------------------------------------------------
OPInstruction msg_cmd(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(core::Data(data, sizeof(cmd)), ArgumentType::ArgType_Byte)
	);
	...
	if (cmd == 75 || cmd == 194)
	{
		...
		instructions.AddArgument(
			OPArgument(
				core::Data(data + 1, pos - 1),
				ArgumentType::ArgType_String
			)
		);
	}
...
//---------------------------------------------------------------------
OPInstruction msg_cmd_v8(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	if (cmd == 209)
	{
		...
		instructions.AddArgument(
			OPArgument(
				core::Data(data + pos, sizeof(uint32_t)),
				ArgumentType::ArgType_Int32
			)
		);
...
//---------------------------------------------------------------------
OPInstruction msg_cmd_he100(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction msg_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction msg_op_v8(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(
			core::Data(data, sizeof(uint32_t)),
			ArgumentType::ArgType_Int32
		)
	);
...
//---------------------------------------------------------------------
OPInstruction actor_ops_v6(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction actor_ops_v8(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction actor_ops_he60(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction verb_ops_v6(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction verb_ops_v8(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction array_ops_v6(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(
			core::Data(data + 1, sizeof(uint16_t)),
			ArgumentType::ArgType_Int16
		)
	);
...
//---------------------------------------------------------------------
OPInstruction array_ops(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	if (cmd == 127)
	{
		instructions.AddArgument(
			OPArgument(
				core::Data(data + pos, sizeof(uint16_t)),
				ArgumentType::ArgType_Int16
			)
		);
	}
	else if (cmd == 138)
	{
		...
		instructions.AddArgument(
			OPArgument(
				core::Data(data + pos + sizeof(uint16_t), sizeof(uint16_t)),
				ArgumentType::ArgType_Int16
			)
		);
	}
...
//---------------------------------------------------------------------
OPInstruction array_ops_v8(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(
			core::Data(data + 1, sizeof(uint32_t)),
			ArgumentType::ArgType_Int32
		)
	);
...
//---------------------------------------------------------------------
OPInstruction array_ops_he100(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction wait_ops(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	if (cmd == 168 || cmd == 226 || cmd == 232)
	{
		instructions.AddArgument(
			OPArgument(
				core::Data(data + 1, sizeof(int16_t)),
				ArgumentType::ArgType_Ref
			)
		);
	}
...
//---------------------------------------------------------------------
OPInstruction wait_ops_v8(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	if (cmd == 30 || cmd == 34 || cmd == 35)
	{
		instructions.AddArgument(
			OPArgument(
				core::Data(data + 1, sizeof(int32_t)),
				ArgumentType::ArgType_Ref
			)
		);
	}
...
//---------------------------------------------------------------------
OPInstruction wait_ops_he100(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction room_ops_he60(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction dmsg_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	instructions.AddArgument(
		OPArgument(
			core::Data(data + start, pos - start),
			ArgumentType::ArgType_String
		)
	);
...
//---------------------------------------------------------------------
OPInstruction sys_msg(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction ini_op_v71(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
//---------------------------------------------------------------------
OPInstruction file_op(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
	...
	if (cmd == 8)
	{
		instructions.AddArgument(
			OPArgument(
				core::Data(data + 1, sizeof(uint8_t)),
				ArgumentType::ArgType_Byte
			)
		);
	}
...
//---------------------------------------------------------------------
OPInstruction file_op_he100(uint8_t byte, const unsigned char* data)
{
	OPInstruction instructions(byte);
...
OPInstruction default_func(uint8_t byte, const unsigned char* data)
{
	return OPInstruction(byte);
}

// ============================================================================
// V6
// ============================================================================
using OPCodeMap = std::map<uint8_t, bytecode>;

//---------------------------------------------------------------------
OPCodeMap GetV6codes()
{
	static OPCodeMap OPCODES_V6 =
	{
...
//---------------------------------------------------------------------
OPCodeMap GetHE60codes()
{
	static OPCodeMap OPCODES_HE60 = GetV6codes();
...
//---------------------------------------------------------------------
OPCodeMap GetHE70codes()
{
	static OPCodeMap OPCODES_HE70 = GetHE60codes();
...
//---------------------------------------------------------------------
OPCodeMap GetHE71codes()
{
	static OPCodeMap OPCODES_HE71 = GetHE70codes();
...
//---------------------------------------------------------------------
OPCodeMap GetHE72codes()
{
	static OPCodeMap OPCODES_HE72 = GetHE71codes();
	...
	// TODO: OPCODES_HE72[0xa0] = { "o72_findObject" };
	...
	// TODO: OPCODES_HE72[0xe3] = { "o72_pickVarRandom" };
...
//---------------------------------------------------------------------
OPCodeMap GetHE73codes()
{
	static OPCodeMap OPCODES_HE73 = GetHE72codes();
...
//---------------------------------------------------------------------
OPCodeMap GetHE80codes()
{
	static OPCodeMap OPCODES_HE80 = GetHE73codes();
	...
	// TODO: OPCODES_HE80[0x4C] = { "o80_sourceDebug" };
	...
	OPCODES_HE80[0xE3] = { "o80_pickVarRandom", extended_w_op };

	return OPCODES_HE80;
}

//---------------------------------------------------------------------
OPCodeMap GetHE90codes()
{
	static OPCodeMap OPCODES_HE90 = GetHE80codes();
	...
	// TODO: OPCODES_HE90[0xab] = { "o90_getActorAnimProgress" };
	// TODO: OPCODES_HE90[0xc8] = { "o90_kernelGetFunctions" };
	// TODO: OPCODES_HE90[0xc9] = { "o90_kernelSetFunctions" };

	return OPCODES_HE90;
}

//---------------------------------------------------------------------
OPCodeMap GetHE100codes()
{
	static OPCodeMap OPCODES_HE100;
	...
	// TODO: OPCODES_HE100[0x22] = { "o6_doSentence" };
	OPCODES_HE100[0x23] = { "o6_drawBox" };
	// TODO: OPCODES_HE100[0x24] = { "o72_drawWizImage" };
	// TODO: OPCODES_HE100[0x25] = { "o80_drawWizPolygon" };
	...
	// TODO: OPCODES_HE100[0x2a] = { "o6_endCutscene" };
	OPCODES_HE100[0x2B] = { "o6_stopObjectCodeObject" };  // o6_stopObjectCode
	OPCODES_HE100[0x2C] = { "o6_stopObjectCodeScript" };  // o6_stopObjectCode
	OPCODES_HE100[0x2D] = { "o6_eq" };
	// TODO: OPCODES_HE100[0x2e] = { "o100_floodFill" };
	// TODO: OPCODES_HE100[0x2f] = { "o6_freezeUnfreeze" };
	...
	// TODO: OPCODES_HE100[0x59] = { "o100_jumpToScriptUnk" };
	OPCODES_HE100[0x5A] = { "o100_startScriptUnk", extended_b_op };
	// TODO: OPCODES_HE100[0x5b] = { "o6_pseudoRoom" };
	...
	OPCODES_HE100[0x60] = { "o6_pushWordVar", extended_w_op };
	...
	// TODO: OPCODES_HE100[0x67] = { "o80_localizeArrayToRoom" };
	...
	// TODO: OPCODES_HE100[0x71] = { "o6_setBoxSet" };
	...
	// TODO: OPCODES_HE100[0x75] = { "o6_delayMinutes" };
	...
	// TODO: OPCODES_HE100[0x7d] = { "o6_startScriptQuick" };
	...
	// TODO: OPCODES_HE100[0x8e] = { "o6_walkActorToObj" };
	...
	// TODO: OPCODES_HE100[0x94] = { "o6_getActorWalkBox" };
	...
	// TODO: OPCODES_HE100[0x9d] = { "o6_getActorWidth" };
	...
	// TODO: OPCODES_HE100[0xa2] = { "o90_getActorAnimProgress" };
	...
	// TODO: OPCODES_HE100[0xad] = { "o71_findBox" };
	// TODO: OPCODES_HE100[0xae] = { "o6_findInventory" };
	OPCODES_HE100[0xAF] = { "o72_findObject" };
	// TODO: OPCODES_HE100[0xb0] = { "o72_findObjectWithClassOf" };
	OPCODES_HE100[0xB1] = { "o71_polygonHit" };
	// TODO: OPCODES_HE100[0xb2] = { "o90_getLinesIntersectionPoint" };
	...
	// TODO: OPCODES_HE100[0xbb] = { "o6_isActorInBox" };
	OPCODES_HE100[0xBC] = { "o6_isAnyOf" };
	// TODO: OPCODES_HE100[0xbd] = { "o6_getInventoryCount" };
	...
	// TODO: OPCODES_HE100[0xc4] = { "o90_getObjectData" };
	...
	// TODO: OPCODES_HE100[0xcc] = { "o72_getPixel" };
	// TODO: OPCODES_HE100[0xcd] = { "o6_distObjectObject" };
	// TODO: OPCODES_HE100[0xce] = { "o6_distObjectPt" };
	// TODO: OPCODES_HE100[0xcf] = { "o6_distPtPt" };
	...
	// TODO: OPCODES_HE100[0xda] = { "o80_getSoundVar" };
	...
	// TODO: OPCODES_HE100[0xe3] = { "o71_concatString" };
...
//---------------------------------------------------------------------
OPCodeMap GetHE101codes()
{
	static OPCodeMap OPCODES_HE101 = GetHE100codes();
...
//---------------------------------------------------------------------
OPCodeMap GetV8codes()
{
	static OPCodeMap OPCODES_V8;
...
//---------------------------------------------------------------------
const OPCodeMap GetOpcodeTable(int version, int heVersion)
{
...
//---------------------------------------------------------------------
bool CheckJumpValidity(const OPInstruction& a_Instruction, size_t a_iCmdPos, const core::Data& a_Data)
{
	size_t cmdSizeTillJump = sizeof(a_Instruction.GetByteCode());
	for (const OPArgument& argument : a_Instruction.GetArguments())
	{
		if (argument.GetArgumentType() == ArgumentType::ArgType_Ref)
		{
...
//---------------------------------------------------------------------
bool ParseScript(const core::Data& a_Data, const OPCodeMap& a_mOPCodes, std::vector<OPInstruction>& a_aInstructions)
{
	...
	while (tell < a_Data.size())
	{
		...
		auto it = a_mOPCodes.find(code);

		assert(it != a_mOPCodes.end());
		if (it == a_mOPCodes.end())
		{
			return false;
		}

		instructionsStrings.push_back(it->second.m_sName);
		OPInstruction instruction = it->second.m_fnSize(code, pureDat + 1);

		assert(CheckJumpValidity(instruction, tell, a_Data));

		a_aInstructions.push_back(instruction);
...
//---------------------------------------------------------------------
bool BuildScript(core::DataStream& a_Data, const OPCodeMap& a_mOPCodes, const std::vector<OPInstruction>& a_aInstructions)
{
	size_t totalSize = 0;
	for (const OPInstruction& instruction : a_aInstructions)
	{
		size_t instructionSize = 0;
		instructionSize += sizeof(uint8_t); // OP code.

		for (const OPArgument& argument : instruction.GetArguments())
		{
...
int main()
{
	...
	OPCodeMap map = GetOpcodeTable(6, 98);
